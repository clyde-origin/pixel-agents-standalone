...
export function AgentFeed({ agentId, folderName, entries, isMobile, onClose, onBack, embedded }: AgentFeedProps) {
  ...
  return (
    <div style={wrapperStyle}>
      <div style={{ background: '#FF7A1A', color: '#1a0a00', padding: '10px 14px', display: 'flex', justifyContent: 'space-between', alignItems: 'center', fontWeight: 700, fontSize: 18 }}>
...
function FeedCard({ entry }: { entry: FeedEntry }) {
  ...
  const card = (color: string, label: string, body: React.ReactNode) => (
    <div style={{ background: '#16181f', border: `1px solid ${color}33`, borderLeft: `5px solid ${color}`, padding: '14px 18px', fontSize: 22, lineHeight: 1.4 }}>
      <div style={{ color: '#7d8694', fontSize: 14, marginBottom: 6 }}>{ts} · {label}</div>
      <div style={{ wordBreak: 'break-word' }}>{body}</div>
    </div>
  )
  switch (entry.kind) {
    case 'text':       return card('#9ad8ff', 'assistant', <span>{entry.text.slice(0, 400)}{entry.text.length > 400 ? '…' : ''}</span>)
